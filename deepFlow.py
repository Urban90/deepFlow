...
# import dependencies from velocity
...
extracted_path = './extracted_path_final'
mag = './MAG'
jpg_path = './jpg/'
out_aorta= './npmin'
out_file = './params.csv'
model_aorta = './AortaHunter_7411.h5'
masks_path_2 = './masks2'
nifti_final = './nifti_final'
didnmake = './didnmake/List.txt'
# In[4]:
...
def extract_data(input_path, extracted_path):
    ...
    with zipfile.ZipFile(input_path, 'r') as zip_ref:
            zip_ref.extractall(os.path.join(extracted_path, splitted))
                #print(os.path.join(extracted_path, x))
            #print('working on extracing data from the zip files :')
    return
        ...
        for subfile in os.listdir(x):
            ...
            if subfile.endswith(".dcm"):
                ...
                try:
                    ...
                    #print('working on changing the names of the files inside the dicom tag: ', subfile)
                except:
...
def loadnii(nii_path):
    ...
    #masked_flipped = flipped*masks*(pix_dim**2) *dt
    net_flow, retrograde_flow, antegrade_flow, regurgitation_fraction, boolean = get_flow(masked)
    #np_sum_rotated = get_flow(masked_rotated)
    net_flow_neg, retrograde_flow_neg, antegrade_flow_neg, regurgitation_fraction_neg, boolean_neg = get_flow(masked_neg)
    #np_sum_flipped = get_flow(masked_flipped)
    #mean_masked = mean_flow(np_sum_masked)
    #mean_masked_final = get_mean(masks, masked)
    #flow_final = get_flow_final(masked)
    #mean_rotated = mean_flow(np_sum_rotated)
    #mean_neg = mean_flow(np_sum_neg)
    #mean_neg_final = get_mean(masks, masked_neg)
    #flow_final_neg = get_flow_final(masked_neg)
    #mean_flipped = mean_flow(np_sum_flipped)
    #aorta_masked = get_AR(np_sum_masked)
    #aorta_rotated = get_AR(np_sum_rotated)
    #aorta_neg = get_AR(np_sum_neg)
    #aorta_flipped = get_AR(np_sum_flipped)
    #plt.imshow(nifti_files_list[0], cmap = plt.cm.bone) 
    #plt.imshow(masks[0], alpha = 0.3, cmap = plt.cm.bone)
    #plt.title('rotated example from the 1st frame')
    #plt.show()
...
def get_ai(nifti_final):
    ...
    for i in os.listdir(nifti_final):
        ...
        nifti_ids, net_flow, retrograde_flow, antegrade_flow, regurgitation_fraction, aorta_perimeter, max_aorta, min_aorta, aorta_area, masks_v_max, masks_v_min = loadnii(path)
...
def get_csv(ids, regurgitation, antegrade_flow_list, retrograde_flow_list, net_flow_list, aorta_perimeter_list, max_aorta_list, min_aorta_list, aorta_area_list, masks_v_max_list, masks_v_min_list):
...
def pipeline(path, extracted_path_example):
    try:
        extract_data(path, extracted_path_example)
    ...
    try:
        ids, regurgitation, antegrade_flow_list, retrograde_flow_list, net_flow_list, aorta_perimeter_list, max_aorta_list, min_aorta_list, aorta_area_list, masks_v_max_list, masks_v_min_list = get_ai(nifti_final)
        print('ar parameters retrieved')
    except:
        print('ar parameters not retrieved')
    try:
        aortic_regurgitation = get_csv(ids, regurgitation, antegrade_flow_list, retrograde_flow_list, net_flow_list, aorta_perimeter_list, max_aorta_list, min_aorta_list, aorta_area_list, masks_v_max_list, masks_v_min_list)
...
with open(didnmake, "a") as failFile:
    for filename in os.listdir(folder_path):
        ...
        if splitted not in filenames_done:
            try:
                #print('something')
                value = pipeline(path, extracted_path)
            except:
                continue
